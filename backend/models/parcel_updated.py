...
class ParcelCategory(Enum):
    ...
    # Nouvelles catégories trouvées dans les données GeoJSON
    ...
    MAISON_DES JEUNES = "Maison des jeunes"
...
class Parcel:
    ...
    def _from_db_row(cls, row):
        ...
        # Gérer la catégorie - priorité à la colonne 'category' si elle existe, sinon utiliser 'dest'
        category_val = row['category'] if row['category'] else row['dest']
        try:
            parcel.category = ParcelCategory(category_val)
        except ValueError:
            # Si la catégorie n'existe pas dans l'énumération, utiliser une valeur par défaut
            parcel.category = ParcelCategory.INDEFINI
